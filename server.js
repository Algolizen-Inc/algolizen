...
// Edit blog post
app.put('/api/blogs/:id', upload.single('image'), express.urlencoded({ extended: true }), async (req, res) => {
  const { id } = req.params;
  const { title, content } = req.body;
  const image = req.file ? `/uploads/${req.file.filename}` : null;
  ...
  try {
    const existing = await db.get(`SELECT * FROM blogs WHERE id = ?`, [id]);
    if (!existing) {
      return res.status(404).json({ error: 'Blog not found.' });
    }

    await db.run(
      `UPDATE blogs SET title = ?, content = ?, image_url = COALESCE(?, image_url) WHERE id = ?`,
